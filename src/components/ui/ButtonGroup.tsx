...
export default function ButtonGroup(props: Props) {
    const {children, direction='vertical', gap=0, style='container'} = props;
    ...
    return (
        <div className={`${styleList[style]} my-4 grid ${direction === 'horizontal' && Array.isArray(children)? `grid-cols-${children.length}`:''} rounded w-full overflow-hidden ${gap? `gap-2 [&>*]:rounded`:''}`}>
            {children}
        </div>
    )
}